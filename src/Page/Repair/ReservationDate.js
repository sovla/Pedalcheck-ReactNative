...
export default function ReservationDate({navigation, route: {params}}) {
    ...
    useEffect(() => {
        if (isFocused) {
            onChangeMonth(selectDate?.substr(0, 7) ?? `${now.getFullYear()}-${now.getMonth() + 1 < 10 ? '0' + (now.getMonth() + 1) : now.getMonth() + 1}`);
        }
    }, [isFocused]);
        ...
        <>
            ...
            <Box flex={1}>
                <ScrollBox flex={1} keyboardShouldPersistTaps="handled">
                    ...
                    <Box mg="20px 16px">
                        <ReservationCalendar selectDate={selectDate} setSelectDate={setSelectDate} onChangeMonth={onChangeMonth} disabledDayList={disabledDayList} />
                    </Box>
                    <TimeList selectDate={selectDate} timeList={timeList} disabled={disabledTimeList} selectItem={selectItem} setSelectItem={setSelectItem} />
