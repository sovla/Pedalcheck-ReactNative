...
export default function Review({isDetail = false, isDetailPage = false, isRecomment = false, width = 'auto', mg = '0px', item = {}, isJustShow = false, onPressDelete, onPressReportHandle}) {
    ...
    const imageArray =
        item?.srt_image?.length > 0
            ...
            : [];
    if ((item?.srt_content?.length >= 90 || item?.srt_res_content?.length >= 90 || imageArray.length > 1 || isLines > 3) && !isDetailButton) {
    ...
    return (
        <Box
            ...
            <BetweenBox width="380px">
                <RowBox>
                    <DefaultImage source={item?.mt_image ? {uri: imageAddress + item?.mt_image} : ProfileDefaultIcon} width="50px" height="50px" />
            ...
            <RowBox mg="10px 0px 0px">
                <DefaultText mg="0px 5px 0px 0px" color={Theme.color.indigo} fontSize={Theme.fontSize.fs15} fontWeight={Theme.fontWeight.bold}>
                    {item?.pt_title}
                </DefaultText>
                {!isCoupon && <MoneyText money={item?.ot_price} color={Theme.color.black} fontSize={Theme.fontSize.fs15} fontWeight={Theme.fontWeight.medium} />}
            </RowBox>
                ...
                {isDetailPage ? (
                    ...
                    imageArray?.length > 0 && (
                        <Box>
                            ...
                            />
                            {imageArray?.length > 1 && (
                                <PositionBox right="10px" bottom="10px" width="44px" height="24px" borderRadius="12px" alignItems="center" backgroundColor="#3336">
            ...
            {isRecomment && (
                <RowBox mg="20px 0px 0px">
                    <ReviewComment reviewDate={item?.srt_adate} reviewContent={item?.srt_res_content} deletePress={isJustShow ? null : () => deletePress(item?.srt_idx)} />
                </RowBox>
            )}

            {isDetailButton && !isDetailPage && (
                <LinkWhiteButton to={() => navigation.navigate('ReviewDetail', {item: item, isRecomment: true})} width="380px" mg="15px 0px 0px 0px" content="자세히보기" />
            )}
        </Box>
    );
}
